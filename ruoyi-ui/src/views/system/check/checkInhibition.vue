<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="checkList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="抑制率标准" align="center" prop="inhibitionNum" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="抑制率标准" prop="inhibitionNum">
          <el-input v-model="form.inhibitionNum" placeholder="请输入抑制率标准" />
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        inhibitionNum: [
          { required: true, message: "抑制率不能为空", trigger: "blur" },
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除检测标准设定编号为"' + ids + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有检测标准设定数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
