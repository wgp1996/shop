      ...
      <el-col :span="1.5">
        ...
        <el-col :span="1.5">
         <el-button
          ...
          :disabled="single"
          @click="rkdConfirm"
          v-hasPermi="['system:order:confirm']"
        >通过</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="rkdCancel"
          v-hasPermi="['system:order:cancel']"
        >退回</el-button>
        ...
        <el-button
          ...
          size="mini"
          @click="handleImport"
          v-hasPermi="['system:cgrkdSingle:import']"
        >导入</el-button>
    ...
    <el-dialog :title="title" :visible.sync="ropen" width="600px">
        <el-form ref="orderForm" :model="orderForm" :rules="rules" label-width="120px">
             <el-form-item label="拒绝原因" prop="isRate" >
              <el-radio-group v-model="orderForm.returnType">
                  <el-radio :label="0" >其他原因</el-radio>
                  <el-radio :label="1">价格太高</el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item label="备注信息" prop="createBy">
              <el-input type="textarea" v-model="orderForm.returnNote" placeholder="请输入备注信息" />
            </el-form-item>
          </el-form>
        <div slot="footer" class="dialog-footer">
            <el-button type="primary" @click="rkdCancelSub">确 定</el-button>
...
import { getShopList } from "@/api/system/shopInfoOwner";
...
export default {
  ...
  data() {
    return {
      ...
      shopList:[],
      fileList: [],
      importUrl:process.env.VUE_APP_BASE_API + "/system/cgrkdSingle/import",
      upload: {
        // 是否显示弹出层（用户导入）
        open: false,
        // 弹出层标题（用户导入）
  ...
  methods: {
   ...
   handleEditShop(data,index, row){
    ...
    handleSelectionChange(selection) {
      ...
      this.orderForm.djNumber=selection.map((item) => item.djNumber)[0];
      this.orderForm.djId=selection.map((item) => item.id)[0];
      this.orderForm.djStatusName=selection.map((item) => item.djStatusName)[0];
    },
    /** 新增按钮操作 */
    handleAdd() {
      this.reset();
    ...
    rkdCancelSub: function () {
      if(this.orderForm.djStatusName!="已接收"){
         this.msgError("禁止操作!");
         return
      }
      if(this.orderForm.returnType==null||this.orderForm.returnType==""||this.orderForm.returnType==undefined){
           this.msgError("请选择拒绝原因类型!");
           return
      }
        rkdCancel(this.orderForm).then((response) => {
          if (response.code === 200) {
    ...
    rkdConfirm: function () {
      ...
      rkdConfirm(this.orderForm).then((response) => {
          if (response.code === 200) {
